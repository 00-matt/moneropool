...
public interface ShareRepository extends JpaRepository<Share, Long> {
    @Query(
            value = "SELECT COALESCE(SUM(difficulty), 0) FROM share WHERE created_at > NOW() - (interval '1' second) * ?1",
            nativeQuery = true
    )
    long findHashesSubmitted(int period);

    @Query(
            value = "SELECT COALESCE(SUM(difficulty), 0) FROM share WHERE miner_id = ?1 AND created_at > NOW() - (interval '1' " +
                    "second) * ?2",
            nativeQuery = true
    )
